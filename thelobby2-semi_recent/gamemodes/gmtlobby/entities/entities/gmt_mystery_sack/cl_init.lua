...
net.Receive("DoSpark",function()
  ...
  if !IsValid(Sack) then return end
  Sack:DoSpark()
end)
...
function ENT:Draw()
	...
	local SackHeight = 10 + math.sin(CurTime() * 2) * 8
  local CurAng = self:GetAngles()

	self.csModel:SetPos(self:GetPos() + Vector(0,0, SackHeight))
	self.csModel:SetAngles(Angle(CurAng.p,CurTime() * 50,CurAng.r + SackAngle))

	render.SetMaterial( self.SpriteMat )
	render.DrawSprite( self.csModel:GetPos() + Vector(0,0,10), 50, 50, self.Color )

  if Glow then
    render.SetMaterial( self.GlowMat )
    render.DrawSprite( self.csModel:GetPos() + Vector(0,0,10), 25 + math.sin(CurTime()*4)*30, 25 + math.sin(CurTime()*4)*30, Color(255,255,50,255) )
  end
