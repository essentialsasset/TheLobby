...
function PANEL:SetHand( hand )
	...
	if !IsValid( self.CardList ) then
		...
		self.CardList:SetSize( w / 5 * 2.2, h )
		self.CardList:SetPos( ( w / 2 ) - ( self.CardList:GetWide() / 2 ), ( h / 2 ) - 32 )
	end

	if hand then

		self.CardList:SetWide( self:GetWide() / 5 * 2.2 )
		self.CardList:SetHand( hand )
		self.CardList:SetPos( self:GetWide() / 2 - self.CardList:GetWide() / 2, h / 2 - 32 )

		// self.CardList:OverrideHand( hand )
		self.Hand = hand
	else
		self.CardList:SetBlankHand( 2 )
	end
...
function PANEL:Paint( w, h )
	...
	if raise && raise > 0 and !selt.Table.Winners then
	...
	// Display hand score
	if self.Hand && self.CardList:IsRevealed() and state == self.Table.States.REVEAL then

		if #self.Community.cards >= 3 then
			
			local handscore = self.Table:GetHandScore( self.Hand, self.Community )
			draw.SimpleText( handscore, "PokerSmall", w/2, h - 10, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )

		else

			draw.SimpleText( "No score", "PokerSmall", w/2, h - 10, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
...
vgui.Register( "HoldemPlayerPanel", PANEL )