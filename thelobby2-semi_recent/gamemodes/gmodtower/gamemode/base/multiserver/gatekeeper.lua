...
gateKeep.HardCodedBans = {
	...
	{"STEAM_0:0:614075224", "tcf"},			-- 'tcf' account	
	-- THE FOLLOWING ARE PEDOPHILES DO NOT EVER UNDER ANY CIRCUMSTANCE UNBAN THEM.
	{"STEAM_0:0:1384695", "kity"},			-- Kity AKA Kate
	{"STEAM_0:0:618033331", "kity"},		-- Kity alt
	{"STEAM_0:0:38865393", "boXy"},			-- boXy AKA Robokid AKA Secretitem AKA Ash
}
...
function gateKeep:CheckPlayerBan(steam, ip)
	...
	local Reason, TotalTime, Time = isPlayerBanned(steam, "steamid") --or isPlayerBanned(ip, "ip")

	if Reason then
		return {false, Reason, TotalTime, Time}
	end
...
hook.Add("PlayerAuthed", "OnlyGMTConnectionsSecondary", function(ply, steamID, UniqueID) --Makes sure that if the first line of security misses, this shouldn't.(At least, not as much)
	...
	if !playerBanned[1] then
		if playerBanned[4] == 0 then
			RunConsoleCommand("kickid", ply:UserID(), "You are banned permanently from "..ServerName.." due to the following: " .. playerBanned[2])
		else
			RunConsoleCommand("kickid", ply:UserID(), "You are banned from "..ServerName.." until (" .. os.date( "%I:%M:%S %p %Z - %m/%d/%Y" , playerBanned[3] ) .. ") due to the following: " .. playerBanned[2])
		end
	end
