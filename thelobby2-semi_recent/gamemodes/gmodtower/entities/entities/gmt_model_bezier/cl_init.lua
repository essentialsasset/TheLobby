...
net.Receive( "gmt_mdlbezier_start", function()
	...
	local pos = util.GetCenterPos( self.GoalEntity )
...
function ENT:Think()
	...
	if self.Model != nil && IsValid( self.GoalEntity ) then
		...
		self.P3 = util.GetCenterPos( self.GoalEntity ) + self.VecOffset

		if !self.P3 then self.P3 = self.GoalEntity:GetPos() + self.VecOffset end
