...
function ENT:DrawWheelParticles( model, pos, ang, MatType, speed )
	...
	if self:IsJumping() || ( self.Trace && !self:HitWorld( self.Trace ) && !RaveMode ) then return end
...
function ENT:Jump( height, downrate )
	...
	if !self:HitWorld( self.Trace ) && !RaveMode then return end
...
function ENT:IsRevved()
	...
	return GAMEMODE:GetState() == STATE_READY && self.RevTime && ( ( CurTime() - self.RevTime ) < .25 )
