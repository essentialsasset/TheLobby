...
function PANEL:UpdatePlayerData()
	...
	if !self:IsVisible() || !self.List:IsVisible() then return end
	
	self:RemovePlayers()
	
	local plys = self.Players
	
	//local validteams = team.GetPlayers( TEAM_PIGS )
	//local ghostPlayers = team.GetPlayers( TEAM_GHOST )
	
	local allPlayers = {}
	
	for _, v in ipairs( team.GetPlayers( TEAM_PIGS ) ) do
		if ( IsValid( v ) && allPlayers[ v:SteamID() ] == nil ) then
			allPlayers[ v:SteamID() ] = v
		end
	end

	for _, v in ipairs( team.GetPlayers( TEAM_GHOST ) ) do
		if ( IsValid( v ) && allPlayers[ v:SteamID() ] == nil ) then
			allPlayers[ v:SteamID() ] = v
		end
	end

	//table.Add( validteams, team.GetPlayers( TEAM_GHOST ) )
