...
function DUELSCREEN:Init()
	...
	local wager = CreateSound( LocalPlayer(), "GModTower/lobby/duel/duel_wager.wav" )
...
function DUELSCREEN:SetPlayers( ply1, ply2 )
	...
	if ( LocalPlayer():GetNWInt( "DuelAmount" ) and tonumber( LocalPlayer():GetNWInt( "DuelAmount" ) ) > 0 ) then
		self.Bet:SetText( "FOR " .. LocalPlayer():GetNWInt( "DuelAmount" ) .. " GMC" )
	end
end
