...
function SidebarPresenter:SetupEvents()
	...
	self:RegisterHook( MP.EVENTS.UI.VOTE_SKIP, function()
		MediaPlayer.Voteskip( mp )
	end )
	
	self:RegisterHook( MP.EVENTS.UI.VOTE_MEDIA, function( value )
		MediaPlayer.VoteMedia( mp, value )
	end )
...
function MediaPlayer.ShowSidebar( mp )
	...
	if Location.IsNightclub( LocalPlayer():Location() ) then
		for k,v in pairs(ents.FindByClass('gmt_club_dj')) do
			if Location.IsNightclub( v:Location() ) then ent = v end
		end
