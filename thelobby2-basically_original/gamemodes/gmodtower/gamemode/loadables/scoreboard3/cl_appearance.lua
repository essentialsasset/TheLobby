...
function APPEARANCE:GenerateModelSelection()
	...
	/*for name, model in pairs( player_manager.AllValidModels() ) do
		...
		if GTowerModels.NormalModels[name] && table.HasValue( LocalPlayer()._ModelTable, model ) then
			AddSpawnIcon( name, model, skin, IconSetModel, CategoryGMTList, 0, 0, "A GMT player model." )
			continue
		end
	...
	AddSpawnIcon( "No Hat", GTowerHats.Hats[0].model, 0, function() RunConsoleCommand('gmt_sethat', '0', '1') end, HatCategoryListHead, 0, 1, "Remove head hat." )

	AddSpawnIcon( "No Hat", GTowerHats.Hats[0].model, 0, function() RunConsoleCommand('gmt_sethat', '0', '2') end, HatCategoryListFace, 0, 2, "Remove face hat." )
	...
	for id, hat in pairs( HatsSorted ) do
		...
		if hook.Call( "CanWearHat", GAMEMODE, LocalPlayer(), hat.unique_Name ) == 1 then
			...
			if hat.slot == 1 then
				...
				AddSpawnIcon( hat.Name, hat.model, hat.ModelSkin, IconSetHatModel, HatCategoryListHead, hat.hatid, hat.slot, hat.description, hat.name )

			else

				AddSpawnIcon( hat.Name, hat.model, hat.ModelSkin, IconSetHatModel, HatCategoryListFace, hat.hatid, hat.slot, hat.description, hat.name )
