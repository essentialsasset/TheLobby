...
hook.Add( "CalcView", "GMTThirdPerson", function( ply, origin, angles, fov )
	...
	if ply.ThirdPerson || ret || !ply:Alive() then
		...
		local pos = ( ply.MonorailModel and ply.MonorailModel:GetPos() or ply:GetPos() ) + Vector( 0, 0, 75 * scale )
		if !ply:Alive() then
		...
		if !ply.ViewingSelf && !ply.MonorailModel then
		...
		if ply.MonorailModel && IsValid(Monorail) then
			ply.CameraAngle = ply.CameraAngle
		end
...
hook.Add( "KeyPress", "ThirdPersonViewSelfKeyPress", function( ply, key )
	...
	if key == IN_ATTACK2 && !ply.ViewingSelf && !Location.Is( LocalPlayer():Location(), "duelarena" ) then
