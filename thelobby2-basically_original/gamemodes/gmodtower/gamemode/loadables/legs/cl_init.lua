...
-----------------------------------------------------
local cl_legs = CreateConVar( "gmt_drawlegs", "1", { FCVAR_ARCHIVE, }, "Enable/Disable the rendering of the legs" )

local cl_legs_vehicle = CreateConVar( "gmt_drawlegs_vehicle", "1", { FCVAR_ARCHIVE, }, "Enable/Disable the rendering of the legs in a vehicle" )
...
local Legs = {
	...
	FixedModelNames = { -- Broken model path = key, fixed model path = value
		...
		["models/player/urban.mdl"] = "models/player/urban.mdl", -- It fucking returns the file type wrong as "mbl" D:
		...
		["models/killingfloor/haroldlott.mdl"] = "models/player/haroldlott.mdl",
...
local function LegsShouldDraw()
	...
	return	ang.p > angLimit and

			cl_legs:GetBool() and

			LocalPlayer().Legs and
...
hook.Add( "UpdateAnimation", "Legs:UpdateAnimation", function( ply, vel, groundSpeed )
	...
	if IsValid( ply.Legs ) then
		...
		if not LegsShouldDraw() then return end
...
hook.Add( "PostDrawTranslucentRenderables", "Legs:Render", function()
	...
	local self = LocalPlayer().Legs


	if not LegsShouldDraw() then -- Should the legs be visible this frame?
