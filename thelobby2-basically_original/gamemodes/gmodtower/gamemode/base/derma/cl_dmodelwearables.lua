...
function PANEL:SetModelWearables( ply )
	...
	if wear1 and wear1 != "models/gmod_tower/no_hat.mdl" then
	...
	if wear2 and wear2 != "models/gmod_tower/no_hat.mdl" then
...
function PANEL:GetTranslations( ent, wear )
	...
	for _, v in pairs( GTowerHats.Hats ) do
		if v.model and v.model == wear:GetModel() then
			hat = v.unique_Name
		end
	end
	...
	local Offsets = GTowerHats:GetTranslation( hat, player_manager.TranslateToPlayerModelName(ent:GetModel()) )
