...
function SCR:CanUse(ply)
	...
	if self.cull and ply:EyeAngles():Forward():Dot(self.forward) > 0 then return false end
...
function SCR:Draw()
	...
	ErrorTrace.Call( self.halt and DrawErr or self.func, function( err, stack )
		self.halt = true

		local Message = err .. "\n"
		surface.PlaySound( "GModTower/ui/panel_error.wav" )

		print( ">>> Screen #" .. self.index .. " errored: " .. err )
		for k, v in pairs( stack ) do
			print( string.rep( " ", k - 1 ) .. k .. ". " .. v )
			Message = Message .. string.rep( " ", k - 1 ) .. k .. ". " .. v .. "\n"
		end
		self.ErrMessage = Message
		DrawErr()

	end, "Screens API", false, self, (self.vw or self.scrw) + fixwidth, self.vh or self.scrh )
	/*
...
if CLIENT then
	...
	hook.Add("PostDrawOpaqueRenderables", "render3d2dScreens", function(depth,sky) if sky then return end Render3DScreens(false) end )
	hook.Add("PostDrawTranslucentRenderables", "render3d2dScreens", function(depth,sky) if sky then return end Render3DScreens(true) end )
...
function New(...) 
	...
	scr.OnMousePressed = function( self, id )
		gui.OpenURL("https://www.kickstarter.com/projects/pixeltailgames/tower-unite")
	end
	...
	scr.Think = function( self )
		...
		local yaw = (self.pos - LocalPlayer():GetPos()):Angle().y --LocalPlayer():EyeAngles().y

		if yaw ~= self.lastYaw and self:GetViewerDistance() > 10000 then
...
/*_GMT_ANNOUNCE_LIGHTS = _GMT_ANNOUNCE_LIGHTS or {}


if CLIENT and IsLobby then

	--EXAMPLE!
	surface.CreateFont( "GTowerHUDHuge2", { font = mainFont2, size = 150, weight = 400 } )

	if not _GMT_ANNOUNCE_LIGHTS[1] then
		_GMT_ANNOUNCE_LIGHTS[1] = ClientsideModel("models/map_detail/nightclub_light.mdl")
		_GMT_ANNOUNCE_LIGHTS[1]:SetPos(Vector(2634, 0, -1005))
		_GMT_ANNOUNCE_LIGHTS[1]:SetAngles(Angle(-90,0,0))
	end

	if not _GMT_ANNOUNCE_LIGHTS[2] then
		_GMT_ANNOUNCE_LIGHTS[2] = ClientsideModel("models/map_detail/nightclub_light.mdl")
		_GMT_ANNOUNCE_LIGHTS[2]:SetPos(Vector(2634, 0, -1005))
		_GMT_ANNOUNCE_LIGHTS[2]:SetAngles(Angle(-90,0,0))
	end

	hook.Add("Think", "FuckingLightsInYourAnusYouBitchAssJiveMontherfucker", function()

		local yaw = 0
		if _GMT_ANNOUNCE_SCREEN then
			yaw = _GMT_ANNOUNCE_SCREEN.lastYaw or 0
		end

		for i=1, 2 do

			local rad = math.rad(yaw + 90)
			local dist = (i==1 and 1 or -1) * 100

			_GMT_ANNOUNCE_LIGHTS[i]:SetPos(Vector(2634 + math.cos(rad)*dist, math.sin(rad)*dist, -1045))
			SetModelScaleVector( _GMT_ANNOUNCE_LIGHTS[i], Vector(4,2,2) )
	...
	if _GMT_ANNOUNCE_SCREEN then
		_GMT_ANNOUNCE_SCREEN:RemoveFromScene()
		_GMT_ANNOUNCE_SCREEN = nil
	end

	WebMat.Get("http://www.towerunite.com",nil,2045,1000)

	local function DrawAnnounceScreen(scr,w,h)
		WebMat.Draw("http://www.towerunite.com",w,h)

		if scr._focused then
			surface.SetDrawColor( Color(255,255,255,80) )
			surface.DrawRect(0,0,w,h)
		end
	end

	if not _GMT_ANNOUNCE_SCREEN then
		_GMT_ANNOUNCE_SCREEN = screen.New( DrawAnnounceScreen, Vector(2634, 0, -910) ):EnableInput(true):TrapMouseWheel(false):TrapMouseButtons(false):SetCull(true):AddToScene(true)
	end

	local thnk = _GMT_ANNOUNCE_SCREEN.Think
	_GMT_ANNOUNCE_SCREEN.Think = function( self )
		thnk( self )
	...
	_GMT_ANNOUNCE_SCREEN.OnFocusState = function( self, state )
		self._focused = state
	end

	_GMT_ANNOUNCE_SCREEN:SetPos( Vector(2634, 0, -910) )
	_GMT_ANNOUNCE_SCREEN:SetMaxDist( 100 )
	_GMT_ANNOUNCE_SCREEN:SetRes( 1920, 1080 )
	_GMT_ANNOUNCE_SCREEN:SetSize( 1920/8, 1080/8 )
	_GMT_ANNOUNCE_SCREEN:SetFade( 200, 400 )
	_GMT_ANNOUNCE_SCREEN:SetFacadeColor( Color(13,24,59) )
	_GMT_ANNOUNCE_SCREEN:SetFacadeMaterial( Material("gui/gradient_down") )
	_GMT_ANNOUNCE_SCREEN:SetBorder(2, Color(255,255,255))

	_GMT_ANNOUNCE_SCREEN.DrawFacade = function(self, fading, depthoverride)
		local c = Color(self.facade_color.r, self.facade_color.g, self.facade_color.b, 255 * fading)
		if depthoverride then render.OverrideDepthEnable( true, true ) end

		surface.SetDrawColor( Color(0,0,0,255 * fading) )
		surface.DrawRect(0,0,self.scrw,self.scrh)		
		...
		local dist = math.sqrt(self:GetViewerDistance())
		local mdist = math.min(1 - math.max(dist - 500, 0) / 300, 1)

		draw.SimpleText("Introducing our new project...", 'GTowerHUDHuge2', self.scrw/2, self.scrh/2, 
			Color(255, 255, 255, 255 * fading * mdist), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)


		if depthoverride then render.OverrideDepthEnable( false, false ) end
	end

	--if not panel.BackBtn then
		--panel.BackBtn = vgui.Create("DButton", panel.HTMLPane)
		--panel.BackBtn:Dock( BOTTOM )
	--end

	--panel.HTMLPane:OpenURL( "https://www.kickstarter.com/projects/pixeltailgames/tower-unite" )
	--panel.HTMLPane:SetAllowLua( true )
	--panel.HTMLPane:OpenURL( "http://towerunite.com/ingame.html?x=1" )

	hook.Add("PlayerBindPress", "announcekeypress", function( ply, bind, pressed )

		if bind == "+use" and pressed then
			if _GMT_ANNOUNCE_SCREEN._focused and _GMT_ANNOUNCE_SCREEN:GetPos():Distance(LocalPlayer():GetPos()) < 1000 then
				surface.PlaySound(Sound( "GModTower/misc/blip.wav" ))
				gui.OpenURL("http://www.towerunite.com")
