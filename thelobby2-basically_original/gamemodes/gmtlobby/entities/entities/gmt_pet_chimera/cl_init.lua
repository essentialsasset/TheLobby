...
CreateClientConVar( "gmt_petname_chimera", "", true, true )
...
function ENT:Draw()
	...
	render.DrawSprite( self:GetPos() + Vector( 0, 0, 10 ), SinBetween( 16, 32, RealTime() ), SinBetween( 16, 32, RealTime() ), Color( math.random( 240, 255 ), math.random( 65, 85 ), math.random( 160, 170 ) ) )
	...
	cam.Start3D2D( pos, Angle( 0, ang.y, 90 ), 0.05 )
		...
		if self:GetPetName() and self:GetPetName() != "" then
			self:DrawText( self:GetPetName(), "PetName", 0, -10, 255, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		end
...
function ENT:Think()
	...
	self.GoalAngle = Angle( 0, ang.y, ang.r )
...
function ENT:DrawParticles()
	...
	if RealTime() > self.NextParticle then
		...
		if particle then
			...
			particle:SetColor( math.random( 240, 255 ), math.random( 65, 85 ), math.random( 160, 170 ) )
