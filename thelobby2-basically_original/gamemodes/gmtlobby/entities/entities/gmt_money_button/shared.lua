...
function ENT:InitSounds()
	...
	for k,v in pairs( self.Sounds ) do self:SoundScript( k ) end
...
function ENT:SoundScript( name )
	...
	self.SoundScripts[name] = Sound( soundname )
...
function ENT:CanUse( ply )
	...
	if self:GetUsePlayer() == ply then
		return false
	elseif self:TestPlayerTrace( ply ) then
		return true, buttonMangle()
	end
