...
function ENT:PlayClick()
	...
	if !self.ClickAmount then self.ClickAmount = 1 end

	if self.ClickAmount == 1 then self.ClickAmount = 2 else self.ClickAmount = 1 end

	if self.ClickAmount%2==0 then
		self:EmitSound( self.SoundClicker2, 60, 100 )
	else
		self:EmitSound( self.SoundClicker1, 60, 100 )
	end
end
...
function ENT:UpdatePositions()
	self.Paddle:SetPos( self:GetPos() + self:GetUp() * (55 + math.sin(UnPredictedCurTime() * 3) * 2 ) )
	self.Wheel:SetPos( self:GetPos() + Vector( 0, 0, 5 + math.sin(UnPredictedCurTime() * 3) * 2 ) )
...
function ENT:DrawTranslucent()
	...
	cam.Start3D2D( pos, ang, .25 )
		...
		draw.TextBackground( label, "PokerText", 0, -22, 8, nil, Color( 150, 50, 0, 150 ) )
