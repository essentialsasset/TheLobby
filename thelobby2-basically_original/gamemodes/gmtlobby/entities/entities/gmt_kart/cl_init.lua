...
function ENT:SetupSounds()
	...
	self.Sounds.Drift = CreateSound( self, "GModTower/sourcekarts/effects/skid_asphalt2.wav" )
	...
	--if not DEBUG then
		self.Sounds.Engine = CreateSound( self, "GModTower/sourcekarts/effects/idle.wav" )
		self.Sounds.Engine:PlayEx( .5, 25 )
	--end
end
...
function ENT:PlayerModelThink( ply )
	...
	if IsValid( self.PlayerModel ) then
		...
		local mdlscale = .5//ScaledModels[ self.PlayerModel:GetModel() ] or 1 --GTowerModels.GetScale( self.PlayerModel:GetModel() )
		...
		if mdlscale < 1 then
			offset = ( ang:Up() * ( mdlscale ) ) + ( ang:Forward() * -16 * ( mdlscale * 1.25 ) )
		end
...
function ENT:FrameThink( trace )
	...
	if IsValid( self.ClientModel ) then
		...
		self.ClientModel:SetModelScale(0.5)

		local angles = self:GetAngles()
		local origin = self:GetPos() - self:GetUp() * 10
		local speed = self:GetSpeed()
...
function ENT:DrawWheelParticles( model, pos, ang, MatType, speed )
	...
	if MatType == "normal" && self:GetIsDrifting() then



		local particle = self.Emitter:Add( "particles/smokey", pos )

		particle:SetVelocity( VectorRand():GetNormal() * 25 + ang )
	...
	local function drawParticles( pos, ang )
		...
		if self:GetIsDrifting() then
			...
			if time > self.Entity.DriftMinimum then
				...
				local scale = math.Clamp( 10 * time, 5, 30 ) * .5
...
function ENT:DrawHeadlights()
	...
	if !self:GetNWBool("FlashOn") then return end
...
if ( ViewDot >= 0 ) then
	...
	local Size = math.Clamp( 2, 64, 512 )

	Distance = math.Clamp( Distance, 32, 800 )
	local Alpha = math.Clamp( 255, 0, 100 )
	local Col = self:GetColor()
	Col.a = Alpha

	local att = self:GetAttachment( self:LookupAttachment( "light_left" ) )
	local pos, attang = att.Pos, att.Ang

	render.DrawSprite( pos, Size, Size, Col, ViewDot )
	render.DrawSprite( pos, Size * 0.4, Size * 0.4, Color( 255, 255, 255, Alpha ), ViewDot )
	...
	local att = self:GetAttachment( self:LookupAttachment( "light_right" ) )
	local pos, attang = att.Pos, att.Ang
...
function ENT:Jump( height, downrate )
	...
	if self:GetOwner() == LocalPlayer() then
		self:EmitSound("gmodtower/sourcekarts/effects/jump.wav",70,125)
	end
