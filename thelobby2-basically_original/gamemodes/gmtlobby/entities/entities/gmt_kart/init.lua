...
concommand.Add("sk_horn",function(ply)
  ...
  ply:GetKart():EmitSound("gmodtower/sourcekarts/effects/horns/horn" .. hornnum .. ".wav" ,60,120)
...
function ENT:Initialize()
	...
	if IsValid(self:GetOwner()) then
		self:GetOwner():SetNWEntity("Kart",self)
	end
...
local _FLASH_ON = false

function ENT:ToggleFlash()
	...
	if !kart.FlashLightOn then kart.FlashLightOn = false end

	kart.FlashLightOn = !kart.FlashLightOn

	kart:SetNWBool("FlashOn",kart.FlashLightOn)

	if on then
kart.flashlight = ents.Create( "env_projectedtexture" )
kart.flashlight:SetParent( kart )

local att = kart:GetAttachment( kart:LookupAttachment( "light_left" ) )
local pos, attang = att.Pos, att.Ang

-- The local positions are the offsets from parent..
kart.flashlight:SetPos( pos )
kart.flashlight:SetAngles( attang )

kart.flashlight:SetKeyValue( "enableshadows", 1 )
kart.flashlight:SetKeyValue( "nearz", 12 )
kart.flashlight:SetKeyValue( "lightfov", math.Clamp( 90, 10, 170 ) )

local dist = 1500
if ( !game.SinglePlayer() ) then dist = math.Clamp( dist, 64, 2048 ) end
kart.flashlight:SetKeyValue( "farz", dist )

local c = Color(255,255,255,255)
local b = 4
if ( !game.SinglePlayer() ) then b = math.Clamp( b, 0, 8 ) end
kart.flashlight:SetKeyValue( "lightcolor", Format( "%i %i %i 255", c.r * b, c.g * b, c.b * b ) )

kart.flashlight:Spawn()

kart.flashlight:Input( "SpotlightTexture", NULL, NULL, "effects/flashlight001" )

kart.flashlight2 = ents.Create( "env_projectedtexture" )
kart.flashlight2:SetParent( kart )

local att = kart:GetAttachment( kart:LookupAttachment( "light_right" ) )
local pos, attang = att.Pos, att.Ang

-- The local positions are the offsets from parent..
kart.flashlight2:SetPos( pos )
kart.flashlight2:SetAngles( attang )

kart.flashlight2:SetKeyValue( "enableshadows", 1 )
kart.flashlight2:SetKeyValue( "nearz", 12 )
kart.flashlight2:SetKeyValue( "lightfov", math.Clamp( 90, 10, 170 ) )

local dist = 512
if ( !game.SinglePlayer() ) then dist = math.Clamp( dist, 64, 2048 ) end
kart.flashlight2:SetKeyValue( "farz", dist )

local c = Color(255,255,255,255)
local b = 4
if ( !game.SinglePlayer() ) then b = math.Clamp( b, 0, 8 ) end
kart.flashlight2:SetKeyValue( "lightcolor", Format( "%i %i %i 255", c.r * b, c.g * b, c.b * b ) )

kart.flashlight2:Spawn()

kart.flashlight2:Input( "SpotlightTexture", NULL, NULL, "effects/flashlight001" )
	...
	kart:EmitSound(Sound(FlashSound))
		...
		if IsValid(kart.flashlight2) then
			kart.flashlight2:Remove()
		end
	end
...
function ENT:StartDrift()
	...
	if (matType == "grass" || matType == "dirt" || matType == "sand") then return end
...
function ENT:GetEndAccel( )
	...
	if self:IsSpinning() then
		return self.CarPhysics.TargetAngular, nil, SIM_LOCAL_ACCELERATION
	end
...
function ENT:FixUpRight()
	...
	if (!self.CarPhysics.GroundTrace || self.CarPhysics.GroundTrace.Fraction > 0.3) && !self:IsOnWater() then
...
function ENT:GetTurnYaw(Driver)
	...
	if Driver:KeyDown(IN_MOVELEFT) then
		return self:GetSetting("TurnPower")--self.SmoothSteerL
	end
	if Driver:KeyDown(IN_MOVERIGHT) then
		return -self:GetSetting("TurnPower")---self.SmoothSteerR
	end
	return 0
end

hook.Add("PlayerSwitchFlashlight","RCCarFlash",function(ply, on)
	if IsValid(ply:GetKart()) then
		ply:GetKart():ToggleFlash()
		return false
	end
end)
