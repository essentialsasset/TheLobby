...
hook.Add("CalcView","TemporarySKCalcView", function( ply, origin, angles, fov )
	...
	if IsValid( kart ) then
		...
		local firstperson = false--( GetConVarNumber( "sk_camera" ) == 2 )
		...
		local dist = camera.FinalDist + math.Fit( speed, 0, 600, 130, 145 ) / 2
		...
		--if speed > 100 && kart.IsInAir && game.GetMap() != "gmt_sk_island02" then
		...
		camera.Roll = ApproachSupport2( camera.Roll, ang.r, 1 )
		...
		// Ease
			...
			camera.Pos.z = ApproachSupport2( camera.Pos.z, Pos.z, 100 )
